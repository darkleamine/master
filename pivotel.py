...
class pivotel:
    ...
    def authentification(self,email,pwd):
        list = []
        list.append('cf '); # cf login -a https://api.run.pivotal.io -u koudjil@live.fr -p ""
        list.append('login -a ');
    ...
    def run(self,email,pwd,nom_app):
        ...
        #os.system('cf login -a https://api.run.pivotal.io -u koudjil10@gmail.com -p "Darkle09&"')
        list = []
        list.append('cf ');
        list.append('login -a ');
        list.append('https://api.run.pivotal.io -u ');
        ...
        try:
            ...
            raise RuntimeError(
                "command apps '{}' return with error (code {}): {}".format(e.cmd, e.returncode, e.output))
        os.system("xdg-open "+nom_app+".cfapps.io")

        return login,deploy,apps

        ################################################################################################################
    def config(self,dirctory,nom_app,command_execute):
        ...
        elif "vendor" in fichiers:
            Fichier.write("  buildpack : https://github.com/cloudfoundry/go-buildpack.git \n")
        elif "Gemfile" in fichiers:
            Fichier.write("  buildpack : https://github.com/cloudfoundry/ruby-buildpack.git \n")
        else:
            pass
        Fichier.write("  command  :  " + command_execute)
